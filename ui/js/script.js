...
/*$(document).ready(function() {
	$('.waterswitcher input').click(function (event) {
		...
		if(target.value == 1) {
			...
			stop_watering_timer($(target).siblings('input')[0]);
		}
	});
...
function createFarm(id) {
	...
	myChart['greenhouse'+id]['tempInterval'] = setInterval(function() {
		if($('#greenhouse'+id+'_tempGraph').is(':visible')) {
			...
			eva_sfa_chart('greenhouse'+id+'_tempGraph','temp','sensor:greenhouse'+id+'/env/temp',{update:30, fill: '10T:1'},myChart['greenhouse'+id]['temp']);
		}
	}, 100);
	...
	myChart['greenhouse'+id]['humInterval'] = setInterval(function() {
		if($('#greenhouse'+id+'_humGraph').is(':visible')) {
			...
			eva_sfa_chart('greenhouse'+id+'_humGraph','hum','sensor:greenhouse'+id+'/env/hum',{update:30, fill: '10T:1'},myChart['greenhouse'+id]['hum']);
		}
	}, 100);
	...
	myChart['greenhouse'+id]['soilInterval'] = setInterval(function() {
		if($('#greenhouse'+id+'_soilGraph').is(':visible')) {
			...
			eva_sfa_chart('greenhouse'+id+'_soilGraph','soilm','sensor:greenhouse'+id+'/env/soilm',{update:30, fill: '10T:1'},myChart['greenhouse'+id]['soil']);
		}
	}, 100);
}
